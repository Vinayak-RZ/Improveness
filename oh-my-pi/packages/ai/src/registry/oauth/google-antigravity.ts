...
function requiredOAuthEnv(name: string): string {
	const value = process.env[name];
	if (!value) {
		throw new Error(
			`${name} must be set; this Improveness tree does not ship upstream OAuth client material.`,
		);
	}
	return value;
}

const CLIENT_ID = requiredOAuthEnv("OMP_GOOGLE_ANTIGRAVITY_CLIENT_ID");
const CLIENT_SECRET = requiredOAuthEnv("OMP_GOOGLE_ANTIGRAVITY_CLIENT_SECRET");
