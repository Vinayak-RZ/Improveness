...
## D16 — Two independently disableable sections: JIT vs Improvement

- **Context:** D15 shipped two-speed (JIT ephemeral + AOT durable) as one product surface. Users need to turn off task-specialized JIT synthesis without killing long-horizon playbook/archive improve, and vice versa. Tool discovery was flat (`ctx.tools`); agents needed hierarchical expand-on-demand and TTSR-inspired inject-when-needed without coupling to Oh My Pi TTSR.
- **Alternatives:** (1) Keep a single always-on loop (rejected: no disable story). (2) Free-form LLM codegen of JIT `apply()` bodies (rejected: fence/disposer risk). (3) Rewrite OMP TTSR into the DSH plugin (rejected: OMP parked; inspiration only). (4) Selected: two load-time sections + template JIT synthesizer + hierarchical ToolCatalog + event inject; dual-horizon improve still gated by `decideAccept`.
- **Selected:** Improveness exposes **JIT** and **Improvement** sections, independently disableable at load time via env (`IMPROVENESS_JIT`, `IMPROVENESS_IMPROVE` parent, `IMPROVENESS_IMPROVE_SHORT`, `IMPROVENESS_IMPROVE_LONG`, `IMPROVENESS_EVENT_INJECT`). Defaults all on. JIT synthesizes task harnesses from typed M/P/A/C **module templates** + archive priors + playbook excerpts — not free-form plugin JS on the hot path. Hierarchical ToolCatalog feeds inspect/expand; SessionEventBus injects tool descriptors on `need_tool` / `tool_fail` / `plan_step` (optional capability mount only when JIT is on and policy allows). Improvement **short-term** proposes after trajectories; **long-term** runs archive/curator cadence; both still pass Self-Harness before durable promote. OMP HostPort catalog parity is P1.
- **Rationale:** Matches the JIT-Agent *split* without training a 27B; keeps HostPort + frozen checker as controller; preserves D7/D15 fences; product README stays claim-honest.

## D17 — ModelTaste: model×harness fit as a third section

- **Context:** Open hosts (DSH, OMP) run many backbones. Apparent “model is bad at tools” failures are often harness-contract failures (malformed tool JSON, unreadable validator dumps, wrong dialect). Command Code / Ahmad Awais “Taste” showed validate-then-repair + teach-back closing much of the DeepSeek tool gap without weight changes. Improveness already has JIT and Improvement (D16) but no model-specific fit layer.
- **Alternatives:** (1) Fold Taste into Improvement (rejected: different promote semantics; must unstrap independently). (2) Patch each host’s tool layer in place (rejected: not portable; violates OMP park). (3) Full preference/taste-file product (deferred P1). (4) Selected: third load-time section + pure package.
- **Selected:** **ModelTaste** is a third Improveness section, disableable via `IMPROVENESS_TASTE=0` (default on). It owns ModelProfile + trace detectors + validate-then-repair plugins + dialect hints + teach-back. Business logic lives in `packages/improveness-modeltaste` (host-agnostic). Hosts strap via HostPort only. Durable promote still goes through `decideAccept` + frozen checker. Inspired by Taste harness-contract work — **not** a port of Command Code preference product.
- **Rationale:** Model×harness fit is the open-host wedge; independent unstrap matches D16 section pattern; claim ledger stays honest (measured fit-suite, not public TB).

## D18 — Modularity / composability: strap-on, zero host kernel edits

- **Context:** User required ModelTaste (and Improveness changes generally) to be modular — easily added to or removed from any system. Patching `oh-my-pi/packages/*` or baking Taste into DSH bundle internals would make uninstall leave residue and block multi-host reuse.
- **Alternatives:** (1) Inline Taste inside `plugins/dsh-improveness/src/` (rejected: not reusable; hard to unstrap). (2) Mutate OMP package sources (rejected: D15 park; upstream license boundary). (3) Selected: pure package + thin HostPort adapters + CI import-graph + unstrap tests.
- **Selected:** `packages/improveness-modeltaste` has **zero** imports from `plugins/*` or `oh-my-pi/packages/*`. DSH and OMP adapters are thin straps (`attachModelProfile` / `detachModelProfile`). With `IMPROVENESS_TASTE=0`, no repair hooks register. Taste must never write into `oh-my-pi/packages/**`. Overlay merge remains Improveness-owned trees only. Same repair logic runs under Bun (core) and is callable from Node (DSH) via JSONL/RPC without duplication.
- **Rationale:** HELIX thin adapter; Cordis spatiotemporal composability; uninstall/clean disable story; publishable library boundary.

## D19 — Parked OMP snapshot refresh policy

- **Context:** `oh-my-pi/` is a D8-owned working snapshot (nested `.git` stripped). It drifted behind `can1357/oh-my-pi`. ModelTaste Qwen dialect work and HostPort parity need a current tree; stale toolconv docs mislead.
...
## D21 — TargetPort: domain kernels and any agentic system

- **Context:** Improveness HostPort is a coding-host adapter (DSH P0, OMP P1). ARC (renamed from Electrical-Engineer) is a **domain kernel**: hosts own the loop; the kernel owns capabilities, recipes, gold, and `unchecked`. Users asked Improveness to become an improving tool for domain kernels and to generalize beyond coding harnesses to any agentic system. Stretching HostPort would leak Fiber/JIT methods into kernels. Vendoring ARC would copy an Apache-2.0 tree into this MIT overlay.
- **Alternatives:** (1) Stretch HostPort (rejected: kernels are not Cordis plugin hosts). (2) Vendor ARC (rejected: license + YAGNI). (3) Port GEPA/DSPy as a second optimizer (rejected: `decideAccept` already is the gate). (4) Selected: sibling TargetPort + JSON manifest + in-tree fixtures.
- **Selected:** `harness/omp/target-port/` loads `targets/<id>/manifest.json` with kind `domain-kernel | agentic-harness | agentic-system`. Same `decideAccept`. Frozen prefixes refuse writes (`assertTargetWrite`). ARC fixture is the motivating kernel; `generic-agentic` proves a non-coding system; `harness-stub` covers the harness kind without replacing HostPort. Live `ARC_ROOT` gold eval and apply onto an external ARC checkout are P1. No public Terminal-Bench as fitness. Grader never writes itself.
- **Rationale:** HELIX thin adapter; Self-Harness / HSI frozen outer anchor; ACE playbooks as the editable surface; ARC’s own Improveness notes (frozen physics, held-out, grader≠improver) become a plug-in contract rather than a DSH mount.
